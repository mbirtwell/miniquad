...
const WM_CUSTOM_EVENT: UINT = WM_USER + 1;
...
unsafe fn _sapp_custom_event(event_data: *mut c_void) {
    if _sapp_events_enabled() {
        _sapp_init_event(sapp_event_type_SAPP_EVENTTYPE_CUSTOM);
...
unsafe extern "system" fn win32_wndproc(
    ...
    if _sapp_win32_in_create_window == false {
        match uMsg {
            ...
            WM_CUSTOM_EVENT => _sapp_custom_event(wParam as *mut c_void),
            _ => {}
        }
    }
